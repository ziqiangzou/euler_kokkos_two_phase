...
namespace euler_kokkos { namespace all_regime
	...
	{
		class ComputeCurvatureHFFunctor2D : HydroBaseFunctor2D
		{
			public:
				ComputeCurvatureHFFunctor2D(HydroParams params_,  DataArray U2data_, DataArray gradphi_):
					HydroBaseFunctor2D(params_), U2data(U2data_),  gradphi(gradphi_) {};
				static void apply(HydroParams params,
						DataArray U2data, DataArray gradphi,
						int nbCells)
				{
					ComputeCurvatureHFFunctor2D functor(params, U2data, gradphi);
				...
				KOKKOS_INLINE_FUNCTION
					void operator()(int index) const
					{
								...
								i>=ghostWidth && i<=params.imax-ghostWidth)
						{
							...
							{
								const real_t phiMx    =U2data(i-1, j,   IH);
								const real_t phiPx    =U2data(i+1, j,   IH);
								const real_t phiMy    =U2data(i,   j-1,   IH);
								const real_t phiPy    =U2data(i,   j+1,   IH);
								if (phiMx*phi0<=ZERO_F||phiPx*phi0<=ZERO_F||phiPy*phi0<=ZERO_F||phiMy*phi0<=ZERO_F)
								{

									int del=1;
									real_t h=del*params.dx;
									if (fabs(gradphi(i, j, IPY))>fabs(gradphi(i, j, IPX)))
									{
										real_t h0=Compute_height_y(i, j,    0, U2data);
										real_t hM=Compute_height_y(i, j, -del, U2data);
										real_t hP=Compute_height_y(i, j,  del, U2data);
										h0=fabs(h0);
										hM=phi0*U2data(i-del, j, IH)>ZERO_F?fabs(hM):-fabs(hM);
										hP=phi0*U2data(i+del, j, IH)>ZERO_F?fabs(hP):-fabs(hP);


										real_t Hmoins=h0-hM;
										real_t HPlus =h0-hP;
										real_t Hmidiu=hM-hP;
										gradphi(i, j, IPK)=2*h*(hP-2.*h0+hM)/sqrt((h*h+Hmoins*Hmoins)*(4*h*h+Hmidiu*Hmidiu)*(h*h+HPlus*HPlus));
									}
									else
									{
										real_t h0=Compute_height_x(i, j,   0, U2data);
										real_t hM=Compute_height_x(i, j,-del, U2data);
										real_t hP=Compute_height_x(i, j,+del, U2data);
										h0=fabs(h0);
										hM=phi0*U2data(i, j-del, IH)>ZERO_F?fabs(hM):-fabs(hM);
										hP=phi0*U2data(i, j+del, IH)>ZERO_F?fabs(hP):-fabs(hP);
										real_t Hmoins=h0-hM;
										real_t HPlus =h0-hP;
										real_t Hmidiu=hM-hP;
										gradphi(i, j, IPK)=2*h*(hP-2.*h0+hM)/sqrt((h*h+Hmoins*Hmoins)*(4*h*h+Hmidiu*Hmidiu)*(h*h+HPlus*HPlus));
		...
		class ComputeRedistancingPhiHopfLaxFunctor2D : HydroBaseFunctor2D
		{
			public:
				ComputeRedistancingPhiHopfLaxFunctor2D(HydroParams params_, DataArray Udata_, DataArray U2data_):
					HydroBaseFunctor2D(params_), Udata(Udata_),U2data(U2data_){};
				static void apply(HydroParams params,
						DataArray Udata, DataArray U2data,
						int nbCells)
				{
					ComputeRedistancingPhiHopfLaxFunctor2D functor(params, Udata, U2data);
					Kokkos::parallel_for(nbCells, functor);
				}
				KOKKOS_INLINE_FUNCTION
					void Projection(DataArray U2data, real_t& xp,real_t& yp, real_t x0, real_t y0, real_t t_k,real_t & phip1, real_t phio) const
					{

						const real_t dx=params.dx;
						real_t dphix, dphiy;
						Compute_LocalValue(xp,yp, phip1, dphix, dphiy, U2data);
						int number=0;
						while(number<6)
						{

							xp-=dphix*dx*phio/(sqrt(dphix*dphix+dphiy*dphiy)*fabs(phio)+1e-8);
							yp-=dphiy*dx*phio/(sqrt(dphix*dphix+dphiy*dphiy)*fabs(phio)+1e-8);

							const real_t distance=sqrt((x0-xp)*(x0-xp)+(y0-yp)*(y0-yp));
							Compute_LocalValue(xp,yp, phip1, dphix, dphiy, U2data);

							if (distance>fabs(t_k))
							{
								xp=x0-(x0-xp)*fabs(t_k)/distance;
								yp=y0-(y0-yp)*fabs(t_k)/distance;
								Compute_LocalValue(xp,yp, phip1, dphix, dphiy, U2data);
								break;
							}
							number++;
				...
				KOKKOS_INLINE_FUNCTION
					real_t secant_method(real_t t_km1, real_t t_k, real_t phi_k, real_t phi_km1) const
					{
						const real_t dx=params.dx;
						real_t dt=-phi_k*(t_k-t_km1)/(phi_k-phi_km1);
						if (fabs(phi_k-phi_km1)==ZERO_F)
						{
							dt=phi_k>ZERO_F?5.*dx:-5.*dx;}
						else if (fabs(dt)>5.*dx)
							dt=dt>ZERO_F?5.*dx:-5.*dx;
						return t_k+dt; 
				...
				KOKKOS_INLINE_FUNCTION
					void operator()(int index) const
					{
						...
						if (j>=ghostWidth && j<=params.jmax-ghostWidth &&
								i>=ghostWidth && i<=params.imax-ghostWidth)
						{
							real_t xp1, yp1, phip1, t_inter,tkp1;
							real_t x1=(i-ghostWidth+HALF_F)*dx+xmin;
							real_t y1=(j-ghostWidth+HALF_F)*dy+ymin;

							real_t phio=Udata(i,j, IH);
							real_t phiL=phio;
							phip1=phio;

							if (fabs(phio)>5)
							{
								tkp1=phio>ZERO_F?0.5*dx:-0.5*dx;
							}
							else
								tkp1=0.5*phio;
							...
							yp1=y1;
							int n=0;
							while (fabs(phip1)>tolerance&&n<params.nx)
							{
								phio=phip1;
								Projection(Udata, xp1, yp1, x1, y1, tkp1,phip1, phiL);

								t_inter=tkp1;
								tkp1=secant_method(tk, t_inter, phip1, phio);
								tk=t_inter;
								n++;

							}
							const real_t dist=std::sqrt((x1-xp1)*(x1-xp1)+(y1-yp1)*(y1-yp1));
							U2data(i, j, IH)=phiL>ZERO_F? dist:-dist;
				...
				KOKKOS_INLINE_FUNCTION
					void operator()(int index) const
					{
								...
								i>=ghostWidth && i<=params.imax-ghostWidth)
						{
							...
							const real_t THREE_F=TWO_F+ONE_F;
