...
namespace euler_kokkos { namespace all_regime
	{
			...
			class SolverHydroAllRegime : public euler_kokkos::SolverBase
		{
			public:
				...
				DataArray     Q2;     /*!< hydrodynamics primitive    variables array  */
				...
				nbCells(params_.isize*params_.jsize)
		{
			...
			if (dim==2)
			{
				...
				total_mem_size += isize*jsize*(3*nbvar+4) * sizeof(real_t);// 1+1+1 for U+U2+Q
			...
			else
			{
				...
				total_mem_size += isize*jsize*ksize*(nbvar*3+5)*sizeof(real_t);// 1+1+1=3 for U+U2+Q
			} // dim == 2 / 3
		...
		template<int dim>
			void SolverHydroAllRegime<dim>::next_iteration_impl()
			{
				...
				{
					if (m_problem_name == "rayleigh_taylor_instabilities")
					{
						real_t mass_total1=ZERO_F;
						real_t mass_total2=ZERO_F;
						PostProcessingFunctors<dim>::ComputeMassConservation1::apply(params, U2, gradPhi, mass_total1, nbCells);
						//PostProcessingFunctors<dim>::ComputeEnergyConservation1::apply(params, U2, gradPhi, Energy1, nbCells);
						PostProcessingFunctors<dim>::ComputeMassConservation2::apply(params, U2, gradPhi, mass_total2, nbCells);
						//PostProcessingFunctors<dim>::ComputeEnergyConservation2::apply(params, U2, gradPhi, Energy2, nbCells);
						std::ofstream location_out;

						location_out.open("Mass_conservation.dat", std::ios::out | std::ios::app); 
						location_out <<m_t<<std::setprecision(std::numeric_limits<long double>::digits10 + 1)<< " " <<mass_total1<<" "<<mass_total2<< "\n"  ;   
						location_out.close();
					}
				...
				if (m_problem_name == "static_bubble")
				{
					...
					location_out.open("ca_32.dat", std::ios::out | std::ios::app); 
					location_out <<m_t*params.settings.mu0/0.64<< " " <<umax*sqrt(0.8/params.settings.sigma)<< "\n"  ;   
					location_out.close();
				}
				if (m_problem_name == "gresho")
				{
					real_t error=ZERO_F;
					PostProcessingFunctors<dim>::ComputeErrorGresho::apply(params, Q, error, nbCells);
					std::ofstream location_out;

					location_out.open("error_gresho.dat", std::ios::out | std::ios::app); 
					location_out <<m_t<< " " <<error/(params.nx*params.ny)<< "\n"  ;   
					location_out.close();
				}
				if (m_problem_name == "transient")
				{
					if (m_iteration % 50==0)
					{
						real_t pC=ZERO_F;
						PostProcessingFunctors<dim>::ComputePressureC::apply(params, Q, pC, nbCells);
						std::ofstream location_out1;
						location_out1.open("Pressure_ProfileC.dat", std::ios::out | std::ios::app); 
						location_out1 <<m_t<< " " <<pC<<"\n"  ;   
						location_out1.close();
					}
				}
				if (((m_problem_name == "Case34"||m_problem_name == "non_isotherm")||m_problem_name == "stefantherm"||m_problem_name == "suckingtherm"))
				{
					if (m_iteration % 50==0)
					{
						real_t positionR=ZERO_F;
						PostProcessingFunctors<dim>::ComputeInterfaceRPosition::apply(params, Q, positionR, nbCells);
						std::ofstream location_out;

						location_out.open("Interface_Position.dat", std::ios::out | std::ios::app); 
						if (m_problem_name == "Case34")
							location_out <<m_t<<" "<<-positionR<<" "<<positionR<<"\n"  ;   
						else if (m_problem_name == "non_isotherm")
							location_out <<m_t<<" "<<(100. - positionR * 1000000.)<<"\n"  ;   
						else
							location_out <<m_t<<" "<<positionR<<"\n"  ;   
						location_out.close();
				...
				{
					if (m_problem_name == "riemann_problem")
					{
						int n=int((m_t+0.001)/0.02);

						PostProcessingFunctors<dim>::ComputeVTProfile::apply(params, Q, n, nbCells);
		...
		template<int dim>
			void SolverHydroAllRegime<dim>::all_regime_scheme()
			{
				...
				RunFunctors<dim>::TransportStep::apply(params, U, Q, U2,gradPhi, m_dt, nbCells);
				// Time state :
				// U  [inner region] : n+1-
				// U2 [inner region] : n+1
				// U  [ghost region] : n+1-
				...
				else
				{
					...
					timers[TIMER_BOUNDARIES]->start();
					make_boundaries_LSC(U);
					timers[TIMER_BOUNDARIES]->stop();

					LevelSetFunctors<dim>::GradientPhiTransport::apply(params, U, gradPhi, nbCells);

					LevelSetFunctors<dim>::TransportPhiStep1::apply(params, U, U2,Q, gradPhi, m_dt, nbCells);

					timers[TIMER_BOUNDARIES]->start();
					make_boundaries_LSC(U2);
					timers[TIMER_BOUNDARIES]->stop();

					LevelSetFunctors<dim>::GradientPhiTransport::apply(params, U2, gradPhi, nbCells);

					LevelSetFunctors<dim>::TransportPhiStep2::apply(params, U, U2,Q, gradPhi, m_dt, nbCells);
					...
					LevelSetFunctors<dim>::TransportPhiStep3::apply(params, U, U2,Q, gradPhi, m_dt, nbCells);
				}
				...
				RunFunctors<dim>::StateChangeStep::apply(params, U2, Q, gradPhi, nbCells);
		...
		template<>
			void SolverHydroAllRegime<2>::init(DataArray Udata)
			{
				...
				{
					InitFunctors<2>::StaticBubble::apply(params, Udata, nbCells);
				}
