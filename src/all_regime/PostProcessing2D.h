...
namespace euler_kokkos { namespace all_regime
	{
		...
		{
			public:
				ComputeEnergyConservation2Functor2D(HydroParams params_, DataArrayConst Udata_, DataArrayConst gradphi_) :
					HydroBaseFunctor2D(params_), Udata(Udata_), gradphi(gradphi_)  {};

				static void apply(HydroParams params, DataArrayConst Udata,DataArrayConst gradphi, real_t& Energy, int nbCells)
				{
					ComputeEnergyConservation2Functor2D functor(params, Udata, gradphi);
							...
							if ((phiLoc*phiMx<=ZERO_F)||(phiLoc*phiPx<=ZERO_F)||(phiLoc*phiMy<=ZERO_F)||(phiLoc*phiPy<=ZERO_F))
							{
								if (fmax(phiLoc, fmax(phiMx, fmax(phiPx, fmax(phiMy, phiPy))))>ZERO_F)
								{
									...
									else
									{
                                                                                ...
                                                                                HydroState uMx = getHydroState(Udata, i-1, j);
                                                                                HydroState uPx = getHydroState(Udata, i+1, j);
                                                                                HydroState uMy = getHydroState(Udata, i, j-1);
                                                                                HydroState uPy = getHydroState(Udata, i, j+1);
                                                                                HydroState u0= compute_GhostState(i, j, uLoc, uMx, uPx, uMy, uPy, dphix, dphiy);

										EnergyLoc =  (ONE_F-surface) * u0[IP] * params.dx*params.dy;
									}
								}
								else
								{
		...
		{
			public:
				ComputeEnergyConservation1Functor2D(HydroParams params_, DataArrayConst Udata_, DataArrayConst gradphi_) :
					HydroBaseFunctor2D(params_), Udata(Udata_), gradphi(gradphi_)  {};

				static void apply(HydroParams params, DataArrayConst Udata,DataArrayConst gradphi, real_t& Energy, int nbCells)
				{
					ComputeEnergyConservation1Functor2D functor(params, Udata, gradphi);
					Kokkos::parallel_reduce(nbCells, functor, Energy);
				}
							...
							if ((phiLoc*phiMx<=ZERO_F)||(phiLoc*phiPx<=ZERO_F)||(phiLoc*phiMy<=ZERO_F)||(phiLoc*phiPy<=ZERO_F))
							{
								if (fmax(phiLoc, fmax(phiMx, fmax(phiPx, fmax(phiMy, phiPy))))>ZERO_F)
								{
									...
									else
									{
                                                                                ...
                                                                                HydroState uPy = getHydroState(Udata, i, j+1);
                                                                                HydroState u0= compute_GhostState(i, j, uLoc, uMx, uPx, uMy, uPy, dphix, dphiy);
										EnergyLoc =  (1-surface) * u0[IP] * params.dx*params.dy;
									}
								}
								else
								{
		...
		{
			public:
				ComputeMassConservation1Functor2D(HydroParams params_, DataArrayConst Udata_, DataArrayConst gradphi_) :
					HydroBaseFunctor2D(params_), Udata(Udata_), gradphi(gradphi_)  {};

				static void apply(HydroParams params,  DataArrayConst Udata,DataArrayConst gradphi, real_t& mass, int nbCells)
				{
					ComputeMassConservation1Functor2D functor(params,Udata, gradphi);
							...
							if ((phiLoc*phiMx<=ZERO_F)||(phiLoc*phiPx<=ZERO_F)||(phiLoc*phiMy<=ZERO_F)||(phiLoc*phiPy<=ZERO_F))
							{
								if (fmax(phiLoc, fmax(phiMx, fmax(phiPx, fmax(phiMy, phiPy))))>ZERO_F)
								{
									...
									else
									{
										...
										massLoc = (ONE_F - surface) * u0[ID] * params.dx*params.dy;
									}
								}
								else
								{
		...
		{
			public:
				ComputeMassConservation2Functor2D(HydroParams params_, DataArrayConst Udata_, DataArrayConst gradphi_) :
					HydroBaseFunctor2D(params_), Udata(Udata_), gradphi(gradphi_)  {};

				static void apply(HydroParams params,  DataArrayConst Udata,DataArrayConst gradphi, real_t& mass, int nbCells)
				{
					ComputeMassConservation2Functor2D functor(params,Udata, gradphi);
					Kokkos::parallel_reduce(nbCells, functor, mass);
				}
