...
namespace euler_kokkos { namespace all_regime
	{
		...
		class ComputeCurvatureHFFunctor3D : HydroBaseFunctor3D
		{
			public:
				ComputeCurvatureHFFunctor3D(HydroParams params_,  DataArray U2data_, DataArray gradphi_):
					HydroBaseFunctor3D(params_), U2data(U2data_),  gradphi(gradphi_) {};
				static void apply(HydroParams params,
						DataArray U2data, DataArray gradphi,
						int nbCells)
				{
					ComputeCurvatureHFFunctor3D functor(params, U2data, gradphi);
					Kokkos::parallel_for(nbCells, functor);
				}
				KOKKOS_INLINE_FUNCTION
					void operator()(int index) const
		...
		class ComputeRedistancingPhiHopfLaxFunctor3D : HydroBaseFunctor3D
		{
			public:
				ComputeRedistancingPhiHopfLaxFunctor3D(HydroParams params_, DataArray Udata_, DataArray U2data_):
					HydroBaseFunctor3D(params_), Udata(Udata_),U2data(U2data_){};
				static void apply(HydroParams params,
						DataArray Udata, DataArray U2data, 
						int nbCells)
				{
					ComputeRedistancingPhiHopfLaxFunctor3D functor(params, Udata, U2data);
					Kokkos::parallel_for(nbCells, functor);
				}
				KOKKOS_INLINE_FUNCTION
					void operator()(int index) const
