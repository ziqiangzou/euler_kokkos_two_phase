...
int main(int argc, char* argv[])
{
  ...
  if (params.nz == 1) {
    ...
    writer.save();
    io::writeXdmfForHdf5Wrapper(params, configMap, 1, var_names, false);
#else
...
#ifdef USE_MPI
    euler_kokkos::io::Load_HDF5_mpi<TWO_D> reader(data, params, configMap, HYDRO_2D_NBVAR, var_names);
#else
    euler_kokkos::io::Load_HDF5<TWO_D> reader(data, params, configMap, HYDRO_2D_NBVAR, var_names);
    reader.load("output2d_0000000.h5");

    configMap.setString("output","outputPrefix","output2d_save");
    {
       euler_kokkos::io::Save_HDF5<TWO_D> writer(data, data_host, params, configMap, HYDRO_2D_NBVAR, var_names, 0, 0.0, "");
       writer.save();
       euler_kokkos::io::writeXdmfForHdf5Wrapper(params, configMap, 1, var_names, false);
    }
...
#ifdef USE_MPI
    euler_kokkos::io::Load_HDF5_mpi<THREE_D> reader(data, params, configMap, HYDRO_2D_NBVAR, var_names);
#else
    euler_kokkos::io::Load_HDF5<THREE_D> reader(data, params, configMap, HYDRO_2D_NBVAR, var_names);
    reader.load("output3d_0000000.h5");

    configMap.setString("output","outputPrefix","output3d_save");
    {
       euler_kokkos::io::Save_HDF5<THREE_D> writer(data, data_host, params, configMap, HYDRO_2D_NBVAR, var_names, 0, 0.0, "");
       writer.save();
       euler_kokkos::io::writeXdmfForHdf5Wrapper(params, configMap, 1, var_names, false);
    }
    // the two files should contain the same data
